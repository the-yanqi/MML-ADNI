...
if __name__ == '__main__':
    ...
    #sigma = 3
    #composed = torchvision.transforms.Compose([GaussianSmoothing(sigma), ToTensor(True)])
    
    s = get_dict(train_path)
    trainset = MriDataset(s,None)
    
    
    s1 = get_dict(test_path)
    testset = MriDataset(s1,None)
    
    
    s2 = get_dict(valid_path)
    validset = MriDataset(s2,None)
    """
    import pickle
    ...
    print("loaded val")
    test = MriDataset(a)
    image2 = test[109]['image']
    testing_image = nib.Nifti1Image(image2.numpy()[0,:], affine=np.eye(4))
    anat = plotting.plot_anat(testing_image, title='subject ' + test[100]['name'])
    anat.savefig('val_pickle_test_1.png')
    print("saved")
    anat.close()
    """
    
    import pickle
    with open("valid_pickle_1", "wb") as f:
         pickle.dump(s2, f)
    print("valid pickle made")
    with open("test_pickle_1", "wb") as f:
         pickle.dump(s1, f)
    print("test pickle made")
    with open("train_pickle_1", "wb") as f:
         pickle.dump(s, f)
    print("train pickle made")
